...
class DesignController{
	...
	public function __construct(Twig $view, Router $router, Messages $flash, Cover $cover, Content $content)
	{
		...
		$view->getEnvironment()->addGlobal('active',['space'=>true,'design'=>true]);
	}
	
	public function index(Request $request, Response $response, Twig $view)
	{
		$grid_num = $this->getGridNum();
		$target_post_route = 'design.create';
		$target_add_content = 'design.add';
		$target_add_tag = 'design.add.tag';
		return $view->render($response, 'space/design.twig',compact('target_post_route','target_add_content','target_add_tag','grid_num'));
	...
	public function create(Request $request, Response $response)
	{
		...
		$pathFor = 'design.index';
		$coverRank = $request->getParam('rank');
		$coverPosition = ($request->getParam('position') == 'design')? '4':'';
		...
		{
			$cover_save_filename = date('ymd').'_'.hash('crc32', $cover_filename).'.'.$cover_fileExt;
	...
	public function add(Request $request, Response $response){
		...
		for($i=0; $i<$count_file; $i++)
		{
			...
			else
			{
				...
				return $response->withRedirect($this->router->pathFor('design.index')); 
			}
		}
	...
	private function getGridNum(){
		$last_cover_rank = $this->cover->where('position','4')->orderBy('rank','desc')->first();
