...
class PeopleController{
	...
	public function __construct(Twig $view, Router $router, Messages $flash, Cover $cover, Content $content)
	{
		...
		$view->getEnvironment()->addGlobal('active',['humanities'=>true,'people'=>true]);
	}
	
	public function index(Request $request, Response $response, Twig $view)
	{
		$grid_num = $this->getGridNum();
		$target_post_route = 'people.create';
		$target_add_content = 'people.add';
		$target_add_tag = 'people.add.tag';
		return $view->render($response, 'humanities/people.twig',compact('target_post_route','target_add_content','target_add_tag','grid_num'));

	}

	public function create(Request $request, Response $response)
	{
		$pathFor_str = 'people.index';
		$coverRank = $request->getParam('rank');
		$coverPosition = ($request->getParam('position') == 'people')? '3':'';
			...
			if($count_content_files > 1)
			{
				for($a=0; $a < $count_content_files; $a++){
					...
					else
					{
						...
						return $response->withRedirect($this->router->pathFor($pathFor_str)); 
					}
				}
			}
		}
	...
	private function getGridNum(){
		$last_cover_rank = $this->cover->where('position','3')->orderBy('rank','desc')->first();
