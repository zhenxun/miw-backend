...
class BuildingController{
	...
	public function __construct(Twig $view, Router $router, Messages $flash, Cover $cover, Content $content)
	{
		...
		$view->getEnvironment()->addGlobal('active',['humanities'=>true,'building'=>true]);
	}
	
	public function index(Request $request, Response $response, Twig $view)
	{
		$grid_num = $this->getGridNum();
		$target_post_route = 'building.create';
		$target_add_content = 'building.add';
		$target_add_tag = 'building.add.tag';
		return $view->render($response, 'humanities/building.twig',compact('target_post_route','target_add_content','target_add_tag','grid_num'));
	...
	public function create(Request $request, Response $response){
		...
		$coverPosition = ($request->getParam('position') == 'building')? '2':'';
			...
			if($count_content_files > 1)
			{
				for($a=0; $a < $count_content_files; $a++){
					...
					else
					{
						...
						return $response->withRedirect($this->router->pathFor('building.index')); 
					}
				}
			}
		}
	...
	private function getGridNum(){
		$last_cover_rank = $this->cover->where('position','2')->orderBy('rank','desc')->first();
