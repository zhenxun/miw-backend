...
class TopicController{
	...
	public function __construct(Twig $view, Router $router, Messages $flash, Cover $cover, Content $content)
	{
		...
		$view->getEnvironment()->addGlobal('active',['furniture'=>true,'topic'=>true]);
	}
	
	public function index(Request $request, Response $response, Twig $view)
	{
		$cover = $this->cover->where('position',5)->first();
		$contents = $this->content->where('cover_id',$cover->id)->orderBy('rank','asc')->get();

		$target_post_route = 'topic.create';
		$target_add_tag = 'topic.add.tag';
		return $view->render($response, 'furniture/topic.twig',compact('target_post_route','target_add_tag','contents'));
	...
	public function create(Request $request, Response $response){
		...
		$num_cover = $this->cover->where('position',5)->count();
		...
		if($count_file > 0)
		{
			...
			for($a=0; $a < $count_file; $a++)
			{
				...
				else
				{
					...
					return $response->withRedirect($this->router->pathFor('topic.index')); 
				}
