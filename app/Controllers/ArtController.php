...
class ArtController{
	...
	public function __construct(Twig $view, Router $router, Messages $flash, Cover $cover, Content $content)
	{
		...
		$view->getEnvironment()->addGlobal('active',['humanities'=>true,'art'=>true]);
	}
	
	public function index(Request $request, Response $response, Twig $view)
	{
		$grid_num = $this->getGridNum();
		$target_post_route = 'art.create';
		$target_add_content = 'art.add';
		$target_add_tag = 'art.add.tag';
		return $view->render($response, 'humanities/art.twig', compact('target_post_route','target_add_content','target_add_tag','grid_num'));
	...
	public function create(Request $request, Response $response){
		...
		$coverPosition = ($request->getParam('position') == 'art')? '1':'';

		$num_checkOldData = $this->cover->where('position',$coverPosition)->where('rank',$coverRank)->count();
		if($num_checkOldData > 0)
		{
			$checkOldData = $this->cover->where('position',$coverPosition)->where('rank',$coverRank)->first();
			$oldCoverid = $checkOldData->id;
		}
		else
		{
			$checkOldData = 0;
				...
				if($count_content_files > 1)
				{
					for($a=0; $a < $count_content_files; $a++){
						...
						if($content_files[$a]->getError() === UPLOAD_ERR_OK)
						{
							...
							$content_save_filename = date('ymdHsi').'_'.hash('crc32', $content_filename).'.'.$content_fileExt;
						...
						else
						{
							...
							return $response->withRedirect($this->router->pathFor('art.index')); 
						}
					}
				}
			}
	...
	private function getGridNum(){
		$last_cover_rank = $this->cover->where('position','1')->orderBy('rank','desc')->first();
