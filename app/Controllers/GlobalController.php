...
class GlobalController{
	...
	public function __construct(Twig $view, Router $router, Messages $flash, Cover $cover, Content $content)
	{
		...
		$view->getEnvironment()->addGlobal('active',['global'=>true]);
	}
	
	public function index(Request $request, Response $response, Twig $view)
	{
		$grid_num = $this->getGridNum();
		$target_post_route = 'global.create';
		$target_add_content = 'global.add';
		$target_add_tag = 'global.add.tag';
		return $view->render($response, 'global/index.twig',compact('target_post_route','target_add_content','target_add_tag','grid_num'));

	}

	public function create(Request $request, Response $response){

		$pathFor = 'global.index';
		$coverRank = $request->getParam('rank');
		$coverPosition = ($request->getParam('position') == 'global')? '7':'';
	...
	public function add(Request $request, Response $response){
		...
		for($i=0; $i<$count_file; $i++)
		{
			...
			else
			{
				...
				return $response->withRedirect($this->router->pathFor('global.index')); 
			}
		}
	...
	private function getGridNum(){
		$last_cover_rank = $this->cover->where('position','7')->orderBy('rank','desc')->first();
